...
st.markdown(
    "Physical simulation of the degradation of optical routing switches in 100G Data Center networks."
)
# --- Sidebar Controls ---
st.sidebar.header("🎛️ Operating Parameters (Physical Parameters)")
temperature = st.sidebar.slider(
    "Server temperature (°C)", min_value=20.0, max_value=70.0, value=35.0, step=1.0
)
max_hours = st.sidebar.slider(
    "Simulation time (hours)", min_value=1000, max_value=12000, value=8000, step=500
)
base_il = st.sidebar.number_input("Base Loss (Base IL dB)", value=1.5)
...
class OCS_DigitalTwin:
    def __init__(self, temp_celsius, base_loss):
        ...
        self.acc_factor = np.exp(
            (0.5 / 8.617e-5) * (1 / (25.0 + 273.15) - 1 / self.temp_k)
        )
    ...
    def advance_time(self, hours):
        ...
        il = (
            self.base_loss
            + 4.34 * ((self.drift_angle / 0.5) ** 2)
            + (self.contamination * 15.0)
        )
        osnr = (0.0 - il) - (-40.0)
...
col1.metric(
    "Total Loss (Max IL)",
    f"{final_il:.2f} dB",
    delta=f"+{final_il - base_il:.2f} dB",
    delta_color="inverse",
)
col2.metric("Signal Quality (Min OSNR)", f"{port.history['osnr'][-1]:.2f} dB")
col3.metric(
    "معدل الخطأ (Final BER)",
    f"{final_ber:.1e}",
    "Catastrophic deterioration" if final_ber > 1e-4 else "Stable",
)
...
if final_il > 8.0:
    st.error(
        "⚠️ Physical Failure: Loss exceeds maximum threshold! Immediate maintenance required."
    )
else:
    st.success("✅ The system is operating within a safe range.")
